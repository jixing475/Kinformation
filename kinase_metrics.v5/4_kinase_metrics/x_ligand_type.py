...
def LigPocketOccupy( pdb_obj ):
  ...
  for res in res_obj:
    ...
    res_type = Tag[0]
    res_id   = Tag[1]
    if re.search(r'H_', res_type): # ' ' = ATOM, 'H_' = HETATM, 'W' = water
      res_name = res_type.split('_')[1]
      # Ignore salt/additive/unnaturalAA heteroatoms
...
#             pock_bp (8) ]
  ...
  if dfg_st == 'in':
    ...
    if Poc[1] is None:
      ...
      if Poc[5] is True or Poc[6] is True or Poc[7] is True or Poc[8] is True:
        lig = 'type-III'
  ...
  # DFG-out and intermediates
    ...
    # If no hinge fragment...
    if Poc[1] is None:
      if Poc[7] is True:    # and with dfg frag...
        lig = 'type-IIA'
      elif Poc[8] is True:  # but has backpocket frag...
        lig = 'type-III'
      elif Poc[5] is True or Poc[6] is True:    # with I1.5 frag but no dfg...
        lig = 'type-II_other'                   # lig points up in dfg pocket
      else:
...
#               pock_I5in,pock_IIb,  pock_dfg,  pock_bp           ]] ... ]
  ...
  for Tgt in Tgt_List:
    ...
    if len(Tgt[7]) > 1:
      Tgt[7].sort(key=lambda x: x[3], reverse=True)
      Data[pdb_id]['lig_multi'] = zip(*Tgt[7])[1]
    Tt = Tgt[7][0]
