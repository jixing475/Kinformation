...
y =plot_ly(y.combined.split.na.omit, x = ~h_cgvc, y = ~p1p1x, z = ~p2p2x, color = ~clust, 
        text = ~paste('Id:', Row.names, '<br>Class:', Class, '<br>Probability:', Probability) )

y= plot_ly(y.combined, x = ~h_cgvc, y = ~p1p1x, z = ~p2p2x, color = ~Class, 
...
y = plot_ly(y.combined.ligand, x = ~h_cgvc, y = ~p1p1x, z = ~p2p2x, color = ~Class, 
        colors = colors,  mode = 'markers', 
        marker = list(symbol = 'circle', sizemode = 'diameter',opacity = .9), sizes = c(10, 40),
        text = ~paste('Id:', Row.names, '<br>Class:', Class, '<br>Probability:', Probability))
...
{
    d= ((as.numeric(x[4])- mid[1])^2 + (as.numeric(x[7])-mid[2])^2 + (as.numeric(x[8])-mid[3])^2 )^.5
    print(d)
}
